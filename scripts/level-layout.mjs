...
for (let i = 0; i < plan.length;) {
	...
	if (type === 10 || type === 11 || type === 16 || type === 17) { actors.push({type,x,z,section}); continue; }
...
const portals = actors.filter(e => e.type === 10 || e.type === 16 || e.type === 17).map(({type,x,z}) => {
	const yaw = (type === 17 ? 0 : type === 16 ? -1 : 1) * Math.PI / 2;
