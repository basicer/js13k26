...
if (typeof code !== 'string') throw Error('Expected a packed release');
...
for (const maxMemoryMB of [32, 150]) {
  ...
  const packedHtml = html.slice(0, match.index) + `<script>${deferGame(packed)}</script>` + html.slice(match.index + match[0].length);
