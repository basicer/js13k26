...
fn vs_post(@builtin(vertex_index) vertex_index: u32) -> @builtin(position) vec4<f32> {
    ...
    return vec4<f32>(position, 0.0f, 1.0f);
}
...
fn fs_post(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    for (var i = 0; i < 25; i++) {
        ...
        let sample = textureLoad(scene_texture, clamp(vec2<i32>(position.xy) + vec2<i32>(offset), vec2<i32>(0), vec2<i32>(dimensions) - vec2<i32>(1)), 0).rgb;
