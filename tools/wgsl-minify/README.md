...
A self-contained lexical WGSL minifier. The implementation is in `src/`, with
unit and compiler-oracle tests in `test/`. It has no runtime dependency on the
game; the integration test reads the parent project's shaders when run there.

The parent Vite build resolves the project's `#import` lines before calling
`src/minify.js`. Development builds keep expanded WGSL source readable.

```sh
npm test                 # Fast local regression tests; no Rust required
npm run test:integration # Compiler comparison against the parent game's shaders
```
