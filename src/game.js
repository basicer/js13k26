...
const weapons = [[12, 1.15, 0, 1], [30, 2.5875, .08, 1], [8, 2.3, .7, 12]];
...
export function fireMarineGun() {
	...
	if (!marineHealth || reloadCooldown > 0 || (DEBUG && player[0] === 255)) return;
	if (weapon[4] === 0) {
	...
	for (let pellet = 0; pellet < weapon[3]; pellet++) {
		...
		const side = radial * Math.cos(angle), vertical = radial * Math.sin(angle);
		firePellet(muzzleX, muzzleY, muzzleZ,
			forwardX * cosine - forwardZ * side,
			forwardZ * cosine + forwardX * side, weapon[3] === 1, vertical);
	}
}
...
function firePellet(muzzleX, muzzleY, muzzleZ, forwardX, forwardZ, assist, forwardY = 0) {
	...
	let range = 18 * shotFraction(muzzleX, muzzleZ, muzzleX + forwardX * 18, muzzleZ + forwardZ * 18,
		assist ? 1.25 : muzzleY, assist ? 1.25 : muzzleY + forwardY * 18);
	...
	for (const unicorn of unicorns) {
		...
		if (distance < range && (!assist || clearShot(muzzleX, muzzleZ, entity[4], entity[6]))
	...
	if (traceX * forwardX + traceZ * forwardZ > 0.05) {
		...
		if (tracer) {
			...
			tracer.set([muzzleX + forwardX * length / 2, muzzleY + forwardY * length / 2, muzzleZ + forwardZ * length / 2], 4);
		...
		if (range > 0 && range < 18) {
			...
			for (let i = 0; i < 8; i++) {
				...
				temporarySphere(
					...
					random(0.1, 0.06), i % 2 ? 248 : 242,
					0, random(0.25, 0.2),
					[-forwardX * speed + random(-0.5),
						random(0.5, 2), -forwardZ * speed + random(-0.5)],
				);
			}
		}
		return;
...
export function setMarineTrigger(held) {
	...
	if (!held) { emptyClickPlayed = false; triggerConsumed = false; }
}

export function reloadMarineGun() {
	if (!marineHealth || reloadCooldown > 0 || weapon[4] === weapon[0] || (DEBUG && player[0] === 255)) return;
