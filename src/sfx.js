	...
	if (!shotgunPumpSamples) {
		...
		for (const [offset, duration, frequency, gain] of [
			...
			for (let i = 0; i < duration * zzfxR; i++) {
				...
				const body =
					Math.sin(2 * Math.PI * frequency * t) * Math.exp(-t * 65);
				const envelope =
					Math.min(t / 0.001, 1) * (1 - t / duration) ** 2;
				shotgunPumpSamples[((offset * zzfxR) | 0) + i] =
					(low + body * 0.7) * envelope * gain * zzfxV;
			}
		}
	}
