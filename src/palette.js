...
const hslToRgb = (h, s, l, a = s * Math.min(l, 1 - l)) =>
	[0, 8, 4].map((n) =>
		255 * (l - a * Math.max(-1, Math.min((n + h * 12) % 12 - 3, 9 - (n + h * 12) % 12, 1))),
	);
...
for (let i = 0; i < 1024; i += 4) for (let c = 0; c < 3; c++) {
	let value = palette[i + c] / 255;
	palette[i + c] = 255 * (value <= 0.04045 ? value / 12.92 : ((value + 0.055) / 1.055) ** 2.4);
}
