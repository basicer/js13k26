...
// Tracker modulation advances with oscillator phase; newer ZzFX advances with sample time.
export var zzfxG = (p=1,k=.05,b=220,e=0,r=0,t=.1,q=0,D=1,u=0,y=0,v=0,z=0,l=0,E=0,A=0,F=0,c=0,w=1,m=0,B=0,N=0,trackerModulation=false)=>{    
    ...
    V=W):0),x=(b+=u+=y)*(trackerModulation?Z(A*H-(A>0?1:-1)*d/4):M.cos(A*H)),H+=trackerModulation?x-x*E*(1-1E9*(Z(a)**2+1)%2):1,g+=x+x*E*Z(a**5),n&&++n>z&&(b+=v,C+=v,n=0),!l||++I%l
    ||(b=C,u=G,n=n||1); return k
}

//! ZzFXM (v2.0.3) | (C) Keith Clark | MIT | https://github.com/keithclark/ZzFXM
// Apply the default frequency before transposing sparse tracker instruments.
export var zzfxM=(n,f,t,e=125,trackerModulation=false)=>{let l,o,z,r,g,h,x,a,u,c,d,i,m,p,G,M=0,R=[],b=[],j=[],k=0,q=0,s=1,v={},w=zzfxR/e*60>>2;for(;s;k++)R=[s=a=d=m=0],t.map((e,d)=>{for(x=f[e][k]||[0,0,0],s|=!!f[e][k],G=m+(f[e][0].length-2-!a)*w,p=d==t.length-1,o=2,r=m;o<x.length+p;a=++o){for(g=x[o],u=o==x.length+p-1&&p||c!=(x[0]||0)|g|0,z=0;z<w&&a;z++>w-99&&u?i+=(i<1)/99:0)h=(1-i)*R[M++]/2||0,b[r]=(b[r]||0)-h*q+h,j[r]=(j[r++]||0)+h*q+h;g&&(i=g%1,q=x[1]||0,(g|=0)&&(R=v[[c=x[M=0]||0,g]]=v[[c,g]]||(l=[...n[c]],l[21]=trackerModulation===true,l[2]=(l[2]??220)*2**((g-12)/12),g>0?zzfxG(...l):[])))}m=G});return[b,j]}
